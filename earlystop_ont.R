...
setwd("G:/My Drive/PhD/Projects/DAISIE/DAISIE_results/results/early_stop/")
...
for (i in seq_along(files)) {
  load(files[i])
  assign(tools::file_path_sans_ext(files[i]), ontogeny_CS_earlystop_ML)
}
ontogeny_CS_earlystop_ML_11 <- ontogeny_CS_earlystop_ML_11[1]
...
for (i in 1:11) {
  ontogeny_CS_df <- rbind(ontogeny_CS_df, do.call(rbind.data.frame, get(paste0("ontogeny_CS_earlystop_ML_", i))))
}
nrow(ontogeny_CS_df)
...
mu_plot_ontogeny <- ggplot(data = final_ont, aes(x = final_ont$mu)) + 
  ...
  ggtitle("Estimates of µ \nwith island ontogeny - early stop") +
...
ont_early_stop_median <- median(final_ont$mu, na.rm = T)
ont_early_stop_sd <- sd(final_ont$mu, na.rm = T)
